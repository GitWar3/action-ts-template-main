...
const today = dayjs_1.default.tz(dayjs_1.default(), 'Asia/Shanghai').format('YYYY年MM月DD日');
const TranslateMap = {
    refactor: '修改',
    fix: '修复',
    feat: '新增',
...
const getCommits = async (token) => await axios_1.default({
    method: 'GET',
...
const pushMessage = async (key, commits) => await axios_1.default({
    ...
    data: {
        msgtype: 'markdown',
        markdown: {
            content: `### ${today}更新总计<font color="red">${commits.length}条</font>\n\n${commits.join('')}`,
        },
    },
});
const filterActions = (commit) => {
    let currentKey = '';
    const flag = Object.keys(TranslateMap).some((key) => {
...
const translateActions = (commit) => {
    const key = filterActions(commit);
...
async function run() {
    try {
        ...
        const { data: commits, status: commitsStatus } = await getCommits(giteeToken).catch(err => err.response);
